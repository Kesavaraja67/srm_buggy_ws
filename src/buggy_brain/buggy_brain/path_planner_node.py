#!/usr/bin/env python3
"""
path_planner_node.py — SRM Autonomous Buggy Path Planner
Publishes: /planned_path (nav_msgs/Path), /navigation_command (std_msgs/String), /planning_status (std_msgs/String)
Subscribes: /navigation_command, /destination_request, /goal_destination
"""
import threading
import rclpy
...
class PathPlannerNode(Node):
    ...
    def __init__(self):
        ...
        # Re-publish path every 2s so late-starting nodes don't miss it
    ...
    def _goal_destination_cb(self, msg: String):
        ...
        with self._lock:
            ...
            if destination == self._current_start:
                ...
                return
            path_nodes = find_shortest_path(EDGES, self._current_start, destination)
            if not path_nodes:
            ...
            nav_cmd = String()
            nav_cmd.data = f'START:{destination}'
    ...
    def _input_loop(self):
        while rclpy.ok():
            ...
            if choice == 'Q':
                print("  [INPUT] Emergency shutdown requested.")
                break
            if choice:
                self._process_destination(choice)
    ...
    def _republish_path(self):
        """Re-send the current path every 2s so late-starting nodes don't miss it."""
        with self._lock:
